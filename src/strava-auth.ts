...
export async function refreshStravaAccessToken(
  ...
  const data: RefreshTokenResponse = await strava.oauth.refreshToken(
    getStravaRefreshToken(settings, botClient)
  );
...
export function getStravaAccessToken(
  ...
  return typeof result === 'string' ? result : settings.stravaAccessToken;
}

export function getStravaRefreshToken(
  settings: Settings,
  botClient: MatrixClient
) {
  const result = (botClient.storageProvider as SimpleFsStorageProvider).readValue(
  ...
  return typeof result === 'string' ? result : settings.stravaRefreshToken;
}
...
export function getStravaAccessTokenExpiresAt(botClient: MatrixClient) {
  return Number(
    (botClient.storageProvider as SimpleFsStorageProvider).readValue(
      accessTokenExpiresAtKey
    )
  );
}
